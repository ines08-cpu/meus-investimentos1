    ...
    for encoding in ['utf-8', 'latin-1', 'utf-16']:
        try:
            ...
            for i, line in enumerate(lines[:20]):
                if any(k in line.lower() for k in ['ticker', 'tipo', 'type', 'montante', 'transação', 'descrição', 'ativo']):
                    skip = i
                    break
...
def process_data(uploaded_files):
    all_assets, m = [], {'dividendos': 0.0, 'juros': 0.0, 'depositos': 0.0}
    ...
    for file in uploaded_files:
        ...
        elif 'ticker' in df.columns and 'valor' in df.columns: # F24
            for _, row in df.iterrows():
                all_assets.append({'Ativo': row['ticker'], 'Cat': 'Ação (F24)', 'Valor': clean_val(row['valor'])})

        # 2. FLUXOS E RENDIMENTOS (Corrigido para evitar os 32k)
        ...
        elif 'transação' in fname or 'numerário' in fname: # F24 Fluxos
            for _, row in df.iterrows():
                desc, valor = str(row.get('transação', '')).lower(), clean_val(row.get('montante', 0))
...
if files:
    ...
    if not df_res.empty:
        ...
        c1.metric("Património", f"€ {total:,.2f}")
        c2.metric("Depósitos", f"€ {metrics['depositos']:,.2f}")
        ...
        st.success(f"**Balanço Global: € {(total - metrics['depositos']) + metrics['dividendos'] + metrics['juros']:,.2f}**")
